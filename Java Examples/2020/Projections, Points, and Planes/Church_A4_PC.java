...
public class Church_A4_PC {
	...
	public static void main(String[] args) throws IOException {
		...
		while (moves.hasNext()) {
			...
			Value x1 = new Value(moves.nextDouble(), moves.nextDouble(), moves.nextDouble()); //x components
			Value y1 = new Value(moves.nextDouble(), moves.nextDouble(), moves.nextDouble()); //Normal (N)
			Value z1 = new Value(moves.nextDouble(), moves.nextDouble(), moves.nextDouble()); //points (Q)
			
			//Plane equation: n1x1+n2x2+n3x3 +c = 0
			//c = -(n1x1+n2x2+n3x3)
			double step1Plane = (x1.getx() * y1.getx());
			double step2Plane = (x1.gety() * y1.gety());
			double step3Plane = (x1.getz() * y1.getz());
			double c = -1*(step1Plane + step2Plane + step3Plane);
		
			//Now we have plane equation, substitute q in for p
			//d = |n1*q1 + n2*q2 + n3*q3 + c| / sqrt(n1^2 + n2^2 + n3^2)
		
			//numerator
			double step1Numerator = (y1.getx() * z1.getx());
			double step2Numerator = (y1.gety() * z1.gety());
			double step3Numerator = (y1.getz() * z1.getz());
			double numerator = step1Numerator + step2Numerator + step3Numerator + c;
		
			//denominator
			double step1Denom = (y1.getx() * y1.getx());
			double step2Denom = (y1.gety() * y1.gety());
			double step3Denom = (y1.getz() * y1.getz());
			double denominator = Math.sqrt(step1Denom + step2Denom + step3Denom);
		
			double distance = numerator / denominator;
			System.out.printf("%.4f\n", distance);
			writer.printf("%.4f\n", distance);
		
		} //end while for moves hasnext
